...
export class ATIComponent extends React.Component<Props, {}> {
    ...
    handleChange(e: React.KeyboardEvent<HTMLButtonElement>) {
        if ($('#modalCloseFirst').is(":focus") && (e.which || e.keyCode == 9)) {
            e.preventDefault();
            $('#modalCloseLast').focus();
        } else if ($('#modalCloseLast').is(":focus") && (e.which || e.keyCode == 9)) {
            e.preventDefault();
            $('#modalCloseFirst').focus();
        }
    }
                ...
                <div className="modal-dialog about-item-modal" role="document">
                    <div className="modal-content">
                        ...
                        <div className="modal-footer">
                            <button className="btn btn-primary" id="modalCloseLast" onKeyDown={this.handleChange} form="accessibility-form" data-dismiss="modal">Close</button>
                        </div>
                    </div>
                </div>
            </div>
